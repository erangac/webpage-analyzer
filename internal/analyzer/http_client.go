...
func (c *httpClient) categorizeNetworkError(err error, urlStr string) (int, string) {
	...
	if strings.Contains(errStr, "no such host") || strings.Contains(errStr, "lookup") {
		return 404, fmt.Sprintf("DNS resolution failed: The domain could not be found. Please check if the URL is correct.")
	}
	
	// Connection refused/timeout errors
	if strings.Contains(errStr, "connection refused") || strings.Contains(errStr, "connect: connection refused") {
		return 503, fmt.Sprintf("Connection refused: The server is not accepting connections. The service might be down or the port might be closed.")
	}
	
	// Network timeout errors
	if strings.Contains(errStr, "timeout") || strings.Contains(errStr, "deadline exceeded") {
		return 408, fmt.Sprintf("Request timeout: The server took too long to respond. Please try again later.")
	}
	
	// SSL/TLS errors
	if strings.Contains(errStr, "certificate") || strings.Contains(errStr, "tls") || strings.Contains(errStr, "ssl") {
		return 495, fmt.Sprintf("SSL/TLS error: There was a problem with the security certificate. The connection is not secure.")
	}
	
	// Protocol errors
	if strings.Contains(errStr, "protocol") || strings.Contains(errStr, "unsupported protocol") {
		return 400, fmt.Sprintf("Protocol error: The URL uses an unsupported protocol. Please use http:// or https://.")
	}
	
	// Network unreachable
	if strings.Contains(errStr, "network is unreachable") || strings.Contains(errStr, "no route to host") {
		return 503, fmt.Sprintf("Network unreachable: Cannot reach the server. Please check your internet connection.")
	}
